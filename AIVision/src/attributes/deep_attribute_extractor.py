...
# COCO Class IDs
PERSON_CLASS_ID = 0
VEHICLE_CLASS_NAMES = {1: "bicycle", 2: "car", 3: "motorcycle", 4: "airplane",
                       5: "bus", 6: "train", 7: "truck", 8: "boat"}
ANIMAL_CLASS_NAMES = {14: "bird", 15: "cat", 16: "dog", 17: "horse", 18: "sheep",
...
class DeepAttributeExtractor:
    def __init__(self, config: dict = None):
        ...
        gender_cfg = self.config.get("attributes", {}).get("gender_model", {})
        if gender_cfg.get("enabled", True):
            try:
                from src.attributes.person_attribute_model import PersonAttributeModel
                self.gender_model = PersonAttributeModel(self.config)
            except Exception as e:
                logger.warning("Failed to initialize PersonAttributeModel: %s", e)
    ...
    def extract(self, crop_bgr: np.ndarray, class_id: int, detection_confidence: float = 1.0,
                class_name: str = None) -> dict:
        """Extract rich attributes for a person, vehicle, animal, or generic object crop,
        ...
        elif class_id in ANIMAL_CLASS_NAMES:
            return self._extract_animal_attributes(crop_bgr, class_id, class_name)
        return self._extract_generic_object_attributes(crop_bgr, class_id, class_name, detection_confidence)

    def _extract_person_attributes(self, crop_bgr: np.ndarray) -> dict:
        h, w = crop_bgr.shape[:2]

        # Upper body (top ~55%) and Lower body (bottom ~45%) split
        upper_region = crop_bgr[0:int(h * 0.55), :]
        lower_region = crop_bgr[int(h * 0.55):h, :]

        # Slicing regions (center 50% width to avoid background) for legacy head and shoe metrics
        w_start = int(w * 0.25)
        w_end = int(w * 0.75)
        head_region = crop_bgr[0:int(h * 0.15), w_start:w_end]
        shoe_region = crop_bgr[int(h * 0.90):h, int(w * 0.3):int(w * 0.7)]

        # 1. Color Extraction with Confidence and Hex using split regions and category context
        ((shirt_color, shirt_hex), shirt_conf) = get_dominant_color(upper_region, self.k, return_confidence=True, return_hex=True, category="upper")
        ((pant_color, pant_hex), pant_conf) = get_dominant_color(lower_region, self.k, return_confidence=True, return_hex=True, category="lower")
        shoe_color, shoe_conf = get_dominant_color(shoe_region, self.k, return_confidence=True, category="lower")
        ...
        bag_val, bag_conf = self._has_bag_with_confidence(upper_region)
        ...
        shirt_type, shirt_type_conf = self._classify_upper_garment(upper_region)
        pant_type, pant_type_conf = self._classify_lower_garment(lower_region)

        # 7. Gender prediction
        gender_val = "unknown"
        gender_conf = 0.0
        if self.gender_model is not None:
            gender_val, gender_conf = self.gender_model.predict_gender(crop_bgr)
        ...
        return {
            ...
            "gender": self._wrap_value(gender_val, gender_conf)
        }
    ...
    def _extract_vehicle_attributes(self, crop_bgr: np.ndarray, class_id: int, detection_confidence: float) -> dict:
        ...
        ((color, color_hex), color_conf) = get_dominant_color(crop_bgr, self.k, return_confidence=True, return_hex=True, category="vehicle")
    ...
    def _extract_animal_attributes(self, crop_bgr: np.ndarray, class_id: int, class_name: str = None) -> dict:
        ((color, color_hex), color_conf) = get_dominant_color(crop_bgr, self.k, return_confidence=True, return_hex=True, category="animal")
        species = ANIMAL_CLASS_NAMES.get(class_id, class_name or "animal")

        return {
            "species": self._wrap_value(species, 1.0),
            "color_name": self._wrap_value(color, color_conf),
            "color_hex": self._wrap_value(color_hex, color_conf),
        }

    def _extract_generic_object_attributes(self, crop_bgr: np.ndarray, class_id: int,
                                            class_name: str = None, detection_confidence: float = 1.0) -> dict:
        ((color, color_hex), color_conf) = get_dominant_color(crop_bgr, self.k, return_confidence=True, return_hex=True, category="object")
        object_type = class_name or str(class_id)

        return {
            "object_type": self._wrap_value(object_type, detection_confidence),
            "color_name": self._wrap_value(color, color_conf),
            "color_hex": self._wrap_value(color_hex, color_conf),
        }
