...
class ClipColorClassifier:
    def __init__(self, clip_embedder):
        ...
        self.device = clip_embedder.device
        self.templates = {
            "upper": "a photo of a {} shirt",
            "lower": "a photo of {} pants",
            "vehicle": "a photo of a {} vehicle",
            "animal": "a photo of a {} animal",
            "object": "a photo of a {} object",
            "generic": "a photo of a {} object"
        }
        self.base_colors = ["black", "white", "gray", "red", "orange", "yellow", "green", "blue", "purple", "pink", "brown"]
        self.fine_colors = list(_COLOR_REFERENCE.keys())
        
        # Precomputed text features: category -> { "fine": (tensor, list), "base": (tensor, list) }
        self.cache = {}
    ...
    def _precompute_all(self):
        import torch
        for cat, template in self.templates.items():
            self.cache[cat] = {}
            # Fine-grained colors
            fine_prompts = [template.format(c) for c in self.fine_colors]
            inputs = self.clip.processor(text=fine_prompts, return_tensors="pt", padding=True).to(self.device)
            with torch.no_grad():
            ...
            self.cache[cat]["fine"] = (features, self.fine_colors)
            
            # Base colors
            base_prompts = [template.format(c) for c in self.base_colors]
            inputs = self.clip.processor(text=base_prompts, return_tensors="pt", padding=True).to(self.device)
            with torch.no_grad():
                features_base = self.clip.model.get_text_features(**inputs)
            features_base = self._extract_tensor(features_base, getattr(self.clip.model, "text_projection", None))
            features_base = features_base / features_base.norm(p=2, dim=-1, keepdim=True)
            self.cache[cat]["base"] = (features_base, self.base_colors)

    def classify(self, crop_bgr: np.ndarray, category: str = "generic", sharpness: float = 200.0) -> tuple:
        import torch
        
        # Choose prompt vocabulary size based on sharpness
        cat = category if category in self.cache else "generic"
        vocab_type = "fine" if sharpness >= _SHARPNESS_MIN_FOR_FINEGRAINED else "base"
        
        text_features, color_names = self.cache[cat][vocab_type]
        
        # Encode image crop
        rgb = cv2.cvtColor(crop_bgr, cv2.COLOR_BGR2RGB)
        inputs = self.clip.processor(images=rgb, return_tensors="pt").to(self.device)
        with torch.no_grad():
        ...
        # Use logit scale from the model to calculate softmax probabilities
        logit_scale = self.clip.model.logit_scale.exp()
        logits = similarities * logit_scale
        probs = torch.softmax(logits, dim=-1)
        
        winning_idx = probs.argmax().item()
        color_name = color_names[winning_idx]
        confidence = float(probs[winning_idx].item())
...
def _get_color_classifier():
    ...
    if _COLOR_CLASSIFIER is None:
        try:
            ...
            clip = get_clip_embedder_instance()
            if clip is not None:
                _COLOR_CLASSIFIER = ClipColorClassifier(clip)
        except Exception as e:
            logger.warning("CLIP color classifier initialization failed: %s", e)
...
def get_dominant_color(image_patch_bgr: np.ndarray, k: int = 3, return_rgb: bool = False, return_confidence: bool = False, return_hex: bool = False, category: str = "generic"):
    ...
    Args:
        ...
        category: the category of the object ("upper", "lower", "vehicle", "animal", "object", "generic").
    ...
    # Now use CIEDE2000 perceptual distance lookup as fallback
    ...
    # Calculate fallback confidence score using CIEDE2000
    proximity_factor = float(np.exp(-ciede_dist / _CIEDE_SIGMA))
    confidence = round(proximity_factor, 2)
    confidence = max(0.0, min(1.0, confidence))

    # Use CLIP zero-shot classification if available
    classifier = _get_color_classifier()
    if classifier is not None:
        try:
            clip_name, clip_conf = classifier.classify(image_patch_bgr, category=category, sharpness=sharpness)
            name = clip_name
            confidence = clip_conf
        except Exception as e:
            logger.warning("CLIP color classification failed, falling back to KMeans+CIEDE2000: %s", e)
