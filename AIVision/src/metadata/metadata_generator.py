...
VEHICLE_CLASS_NAMES = {"bicycle", "car", "motorcycle", "airplane", "bus", "train", "truck", "boat"}
ANIMAL_CLASS_NAMES = {"bird", "cat", "dog", "horse", "sheep", "cow", "elephant", "bear", "zebra", "giraffe"}


def _categorize(class_name: str) -> str:
    """Bucket a detector class name into one of the four top-level description categories."""
...
class MetadataGenerator:
    ...
    def finalize(self, trajectory_tracker=None, motion_analyzer=None,
                 ...
                 deep_attributes_dict=None, keyframe_quality_dict=None,
                 vlm_descriptions_dict=None, fps: float = 30.0) -> list:
        ...
        for track_key, record in list(self._tracks.items()):
            ...
            category = _categorize(class_name)
            ...
            if category == "person":
                ...
                confidence_record["gender"] = confidence_record.get("gender", 0.0)
                ...
                appearance_normalized = {
                    "species": appearance.get("species", class_name),
                    "color": appearance.get("color_name", "unknown"),
                }

                animal_obj = {
                    "species": appearance.get("species", class_name),
                    "color_name": appearance.get("color_name", "unknown"),
                    "color_hex": appearance.get("color_hex", "unknown"),
                    "color_confidence": confidence_record.get("color_name", 0.0)
                }

                confidence_record["species"] = confidence_record.get("species", 1.0)
                confidence_record["color_name"] = confidence_record.get("color_name", 0.0)

            else:  # generic object / "thing"
                appearance_normalized = {
                    "object_type": appearance.get("object_type", class_name),
                    "color": appearance.get("color_name", "unknown"),
                }
                ...
                confidence_record["object_type"] = confidence_record.get("object_type", 1.0)
                confidence_record["color_name"] = confidence_record.get("color_name", 0.0)
            ...
            # 7b. Open-ended VLM caption (not bounded by the fixed attribute schema above)
            vlm_description = None
            if vlm_descriptions_dict and track_key in vlm_descriptions_dict:
                vlm_description = vlm_descriptions_dict[track_key]
            ...
            final_record = {
                ...
                "category": category,
                "upper_body": upper_body_obj if category == "person" else None,
                "lower_body": lower_body_obj if category == "person" else None,
                "vehicle": vehicle_obj if category == "vehicle" else None,
                "animal": animal_obj if category == "animal" else None,
                "object": object_obj if category == "object" else None,
                "gender": record.get("gender", appearance.get("gender", "unknown")) if category == "person" else None,
                "gender_confidence": confidence_record.get("gender", 0.0) if category == "person" else None,
            ...
            final_record["description"] = self._build_rich_description(
                class_name, final_record.get("category", category), final_record, final_record["motion"],
    ...
    def _article_for(word: str) -> str:
        """Return 'An' or 'A' depending on whether `word` starts with a vowel sound."""
        return "An" if word and word[0].lower() in "aeiou" else "A"

    @staticmethod
    def _build_rich_description(class_name: str, category: str, record: dict, motion: dict,
        ...
        animal = record.get("animal", {}) if isinstance(record, dict) and record.get("animal") else {}
        obj = record.get("object", {}) if isinstance(record, dict) and record.get("object") else {}
        ...
        if category == "person":
            ...
            desc = MetadataGenerator._article_for(color if color != "unknown" else vtype)
            if color != "unknown":
                desc += f" {color}"
            ...
            color = animal.get("color_name", "unknown")
            species = animal.get("species", class_name)

            desc = MetadataGenerator._article_for(color if color != "unknown" else species)
            if color != "unknown":
                desc += f" {color}"
            desc += f" {species}"
            ...
            if direction not in ("stationary", "unknown"):
                desc += f", moving {speed} towards the {direction}"
            ...
            color = obj.get("color_name", "unknown")
            otype = obj.get("object_type", class_name)

            desc = MetadataGenerator._article_for(color if color != "unknown" else otype)
            if color != "unknown":
                desc += f" {color}"
            desc += f" {otype}"
            desc += " was detected"
            ...
            desc += f", visible for a total duration of {duration} seconds."
            return desc.replace("  ", " ").replace(" ,", ",")
    ...
    def save_csv(self, output_path: str, finalized_records: list = None) -> None:
        ...
        fieldnames = [
            "track_id", "camera_id", "class", "category", "detected_type", "vehicle_type", "vehicle_number", "color",
            "upper_body", "lower_body", "first_seen", "last_seen",
            "duration", "zone", "events", "group_size", "near_vehicle", "description", "vlm_description",
        ]
        with open(output_path, "w", newline="") as f:
            ...
            for r in records:
                ...
                detected_type = (vehicle.get("vehicle_type") or animal.get("species")
                                 or obj.get("object_type") or ("person" if r["class"] == "person" else r["class"]))
                color_name = (vehicle.get("color_name") or animal.get("color_name") or obj.get("color_name")
                              or upper.get("color_name") or r.get("appearance", {}).get("color", "unknown"))
                ...
                flat_row = {
                    ...
                    "vehicle_type": v_type if r.get("category") == "vehicle" else "",
                    ...
                    "vlm_description": r.get("vlm_description") or "",
                }
