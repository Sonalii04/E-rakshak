...
class MetadataValidator:
    def __init__(self, config: dict):
        ...
        self.gender_conf_threshold = gender_cfg.get("min_confidence", 0.60)
    ...
    def validate_record(self, record: dict, keyframe_metrics: dict = None) -> tuple:
        ...
        if is_too_small or quality_fails or high_occlusion:
            ...
            if record.get("animal"):
                record["animal"]["species"] = "unknown"
                record["animal"]["color_name"] = "unknown"
                record["animal"]["color_hex"] = "unknown"
                record["animal"]["color_confidence"] = 0.0
            if record.get("object"):
                record["object"]["object_type"] = "unknown"
                record["object"]["color_name"] = "unknown"
                record["object"]["color_hex"] = "unknown"
                record["object"]["color_confidence"] = 0.0
        else:
            ...
            if class_name == "person":
                ...
                # Validate gender confidence with custom threshold
                if record.get("gender") and record.get("gender") != "unknown":
                    if confidence.get("gender", 0.0) < self.gender_conf_threshold:
                        record["gender"] = "unknown"
            elif record.get("vehicle"):
            ...
            elif record.get("animal"):
                animal = record["animal"]
                if confidence.get("species", 0.0) < self.attr_conf_threshold:
                    animal["species"] = "unknown"
                if confidence.get("color_name", 0.0) < self.attr_conf_threshold:
                    animal["color_name"] = "unknown"
                    animal["color_hex"] = "unknown"
            elif record.get("object"):
                obj = record["object"]
                if confidence.get("object_type", 0.0) < self.attr_conf_threshold:
                    obj["object_type"] = "unknown"
                if confidence.get("color_name", 0.0) < self.attr_conf_threshold:
                    obj["color_name"] = "unknown"
                    obj["color_hex"] = "unknown"
